...
treasuremap_t *treasuremap_load_text(char *file_name) {
    ...
    // TODO: Check if the file fails to open and return NULL if so. [DONE]
    if (file_handle == NULL) {
        printf("Couldn't open file '%s', returning NULL\n", file_name);

        // TODO: return failure value [DONE]
        return NULL;
    }

    printf("Allocating map struct\n");

    // TODO: Determine byte size for treasuremap_t struct [DONE]
...
// REQUIRED: De-allocate the space associated with a treasuremap_t.
