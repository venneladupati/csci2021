...
// Adds together each element from vectors 'a' and 'b' and stores result in 'c'
...
int main() {
    ...
    int *c = malloc(VEC_LEN * sizeof(int));
    int *d = malloc(VEC_LEN * sizeof(int));

    clock_gettime(CLOCK_PROCESS_CPUTIME_ID, &start);
    array_add(a, b, c, VEC_LEN);
    clock_gettime(CLOCK_PROCESS_CPUTIME_ID, &end);
    printf("array_add: %.3f sec.\n", get_elapsed_time_sec(&start, &end));

    clock_gettime(CLOCK_PROCESS_CPUTIME_ID, &start);
    array_add_vec(a, b, d, VEC_LEN);
    clock_gettime(CLOCK_PROCESS_CPUTIME_ID, &end);
    printf("array_add_vec: %.3f sec.\n", get_elapsed_time_sec(&start, &end));

    if (!array_equals(c, d, VEC_LEN)) {
